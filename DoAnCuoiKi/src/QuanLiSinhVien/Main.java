...
//    	quanLySinhVien.docDanhSachSinhVienTuFile(FILE_NAME);
    ...
    private static void hienThiDanhSachSinhVien() {
        ...
        // Định dạng bảng với các cột mở rộng cho môn học
        String headerFormat = "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n";
        String headerRowFormat = "| STT | Lớp        | Mã Sinh Viên  | Tên Sinh Viên        | Ngày Sinh    | Giới Tính  | Số Điện Thoại          | Email                          | Môn Học Mã  | Môn Học Tên                  | Điểm   |\n";
        String dataFormat = "| %-3d | %-10s | %-13s | %-20s | %-10s   | %-10s | %-20s   | %-30s | %-10s | %-30s | %-5s |\n";

        // Hiển thị tiêu đề bảng
        System.out.printf(headerFormat);
        System.out.printf(headerRowFormat);
        System.out.printf(headerFormat);
        ...
        for (int i = 0; i < danhSachSinhVien.size(); i++) {
            ...
            // Chuẩn bị dữ liệu cho môn học
            List<MonHoc> danhSachMonHoc = sv.getDanhSachMonHoc();

            // Nếu có môn học
            if (!danhSachMonHoc.isEmpty()) {
                for (int j = 0; j < danhSachMonHoc.size(); j++) {
                    MonHoc monHoc = danhSachMonHoc.get(j);
                    if (j == 0) {
                        // Hiển thị thông tin sinh viên chỉ một lần cho môn học đầu tiên
                        ...
                        // Hiển thị thông tin môn học cho các môn học tiếp theo
                        System.out.printf(dataFormat,
                                0, // Chỉ để hiển thị 0 cho STT trong các hàng môn học tiếp theo
                                "",
                                "",
                                "",
                                "",
                ...
                // Hiển thị sinh viên không có môn học
                System.out.printf(dataFormat,
                        i + 1,
                        sv.getLop(),
                        sv.getMaSV(),
                        ...
                        sv.getEmail(),
                        "N/A", // Môn học mã
                        "Chưa đăng ký môn học", // Môn học tên
                        "N/A"  // Điểm
                );
            }

            // Kết thúc phần hiển thị cho mỗi sinh viên
            System.out.printf(headerFormat);
        }
    }
